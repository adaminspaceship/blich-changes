...
class S(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        object3 = object1["result"]["metadata"]["intentName"]
        ...
        if object2 == "today":
            ...
            if "yes" in answerArray:
                ...
                if lang == "he":
                    cancelnum = answerArray.index("yes")+1
                    lastjson = json.dumps({'speech':"כן, בשעה %s"%cancelnum,'displayText':"כן",'data':{},'contextOut':[],'source':'adam'})
                    print("Response: "+lastjson)
                    self.wfile.write(lastjson.encode('utf-8'))
                else:
                    cancelnum = answerArray.index("yes")+1
                    lastjson = json.dumps({'speech':"Yes, lesson number %s"%cancelnum,'displayText':"Yes, lesson number %s",'data':{},'contextOut':[],'source':'adam'})
                    print("Response: "+lastjson)
                    self.wfile.write(lastjson.encode('utf-8'))
            else:
                self._set_response()
        ...
        if object2 == "tomorrow":
            ...
            if "yes" in answerArrayT:
                ...
                if lang == "he":
                    ...
                    print("Response: "+lastjson)
                    self.wfile.write(lastjson.encode('utf-8'))
        #end of canceled lesson check
        tests = []
        if object3 == "tests":
            driver.get("http://blich.iscool.co.il/DesktopModules/IS.TimeTable/MainHtmlExams.aspx?pid=17&mid=6264&layer=1&cls=19")
            testDate = driver.find_element_by_xpath("/html/body/form[@id='form1']/div[@id='ChangesList']/div[@class='ChangesBox']/table/tbody/tr[2]/td[1]")
            #testSubject = driver.find_element_by_xpath("/html/body/form[@id='form1']/div[@id='ChangesList']/div[@class='ChangesBox']/table/tbody/tr[2]/td[2]")
            #testGroup = driver.find_element_by_xpath("/html/body/form[@id='form1']/div[@id='ChangesList']/div[@class='ChangesBox']/table/tbody/tr[2]/td[3]")
            lastjson = json.dumps({'speech':testDate.text,'displayText':testDate.text,'data':{},'contextOut':[],'source':'adam'})
            print("Response: "+lastjson)
            self.wfile.write(lastjson.encode('utf-8'))
